...
namespace CS3358_SP2016_Assign08
{
   ...
   //       NOTE: The default argument for message is the empty string.
   {
    ...
    void p_queue::pop()
    {
        ...
        {
            size_type Index = 0, oldIndex = 0;
            heap[Index] = heap[used-1];
            while(!is_leaf(Index) && heap[Index].priority <= big_child_priority(Index))
            {
                oldIndex = big_child_index(Index);
    ...
    p_queue::size_type
        ...
        //       than that of the other child, if there is one.)
        {
            ...
            if ((2*i+2 >= used)||(heap[2*i+1].priority >= heap[2*i+2].priority))
                return 2*i+1;
            return 2*i+2;
        }
